...
@export var kinds_of_pickup : Array[Supply]
@export var repeat_spawn_curve : Curve

var available_pickups: Array[Pickup]:
	get:
		var pickups: Array[Pickup] = []
		for pickup: Node2D in get_children():
			if pickup is Pickup:
...
func how_many_of_supply_are_spawned(supply: Supply) -> int:
	var count: int = 0
	for pickup: Pickup in available_pickups:
...
func _process(_delta: float) -> void:
	if get_children().size() < minimum_pickups:
		spawn_pickup()

func choose_random_pickup_type() -> Supply:
	var spawn_rate_dict: Dictionary = {}
	
	for supply: Supply in kinds_of_pickup:
		var spawn_rate: float = supply.pickup_spawn_rate
		if available_pickups.size() > 0:
			var percent_of_pickup_population: float = how_many_of_supply_are_spawned(supply) as float / available_pickups.size()
